...
| 3 | Cross-source coincidental identical KO/EN | Not directly measured (data-recon profiled per-dataset, not cross-dataset) | **REVIEW_REQUIRED** | A `duplicate_group_id` spanning two different `source_id`s is legitimate (different provenance, possibly different domain/license) but silently picking one Analysis Representative would erase a source/domain data point; needs an explicit domain-aware representative-selection rule before this can be mechanized |
...
**New open question surfaced by this analysis (route to decision queue)**: is 025's high duplicate-pair rate (~8% of unique-content records, disproportionately concentrated 214,252/2,700,345) explained by TL1/TL2 direction-mirroring (expected, benign) or by genuine accidental repetition within one direction (a data-quality problem)? This reconciliation cannot answer it from the current manifest alone — it requires a targeted check (do duplicate pair-hashes cluster specifically across the TL1↔TL2 file boundary, or also occur within a single file?) that belongs to Raw EDA, not this pre-G1 document.
